...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 4 {
        eprintln!("usage: context_breakdown <instructions.txt> <tools.json> <rollout.jsonl>");
        std::process::exit(2);
    }
    let instructions = fs::read_to_string(&args[1])?;
    let tools_json: Vec<Value> = serde_json::from_str(&fs::read_to_string(&args[2])?)?;
    ...
    println!(
        "  total         : {:>8}  / window {:?}",
        b.total_tokens, b.context_window
    );
    println!();
